...
// Enhanced qualification checking with transcript data
export const performTranscriptQualificationCheck = (transcriptData, studentData = {}) => {
    try {
        const { subjects, grades, gpa, qualificationLevel, englishProficiency } = transcriptData

        // Merge transcript data with provided student data
        const mergedData = {
            ...studentData,
            gpa: gpa || studentData.gpa,
            qualificationLevel: qualificationLevel || studentData.qualificationLevel,
            subjects: subjects || studentData.subjects || [],
            englishProficiency: englishProficiency !== undefined ? englishProficiency : studentData.englishProficiency
        }

        // Perform standard qualification check
        const baseCheck = performOfflineQualificationCheck(mergedData)
        ...
        return {
            ...
            status: "transcript_check_completed"
        }
    } catch (error) {
        return {
            timestamp: new Date().toISOString(),
            error: error.message,
            status: "transcript_check_failed"
        }
    }
}

// Validate qualification for course application
export const validateCourseApplication = (studentProfile, courseName, institutionId) => {
    const check = checkQualification(studentProfile, courseName, institutionId)

    return {
        canApply: check.qualified,
...
// Check if student can apply to multiple courses (max 2 per institution)
export const validateMultipleApplications = (applications, newApplication) => {
    const { institutionId, courseName } = newApplication

    // Count existing applications for this institution
    ...
    if (institutionApps.length >= 2) {
        return {
            valid: false,
            reason: "Maximum 2 course applications allowed per institution",
            currentCount: institutionApps.length
        }
    }
    ...
    if (duplicateApp) {
        return {
            valid: false,
            reason: "Already applied to this course",
            duplicateApplication: duplicateApp
        }
    }
